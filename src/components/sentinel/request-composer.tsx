  ...
  return <section id="request" className="composer request-surface input-surface" aria-labelledby="request-heading">
    <form onSubmit={event => { event.preventDefault(); if (valid && !isRunning && !unavailable) onSubmit(); }}>
      <div className={`prompt-shell ${isRunning ? "working" : ""}`}>
        <div className="composer-heading"><div><span className="input-step-label">START WITH AN IDEA</span><label htmlFor="mission-prompt" id="request-heading">What are you looking for?</label></div><span className="input-region">Canada · CAD</span></div>
    ...
    <p className="composer-assurance"><LockKeyhole size={13} />You’re in control. Nothing runs until you submit. No real purchases.</p>
  </section>;
}
