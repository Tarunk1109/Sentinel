...
const exampleHints = ["Broken chair wheel", "Laptop charger", "Printer toner", "Appliance filter"];
const ACCEPT = "image/jpeg,image/png,image/webp";

export function InspectUpload({ file, previewUrl, validationError, onSelectFile, onRemove, onAnalyze, isAnalyzing, disabled, disabledReason }: {
  ...
  return <section id="inspect-upload" className="composer inspect-composer" aria-labelledby="inspect-heading">
    <div className="inspect-heading"><h2 id="inspect-heading"><ScanLine size={16} />Show SENTINEL what needs attention</h2><p>Upload a photo of something broken, missing, damaged, or needing replacement.</p></div>
    {!file ? <>
      <div
      ...
      >
        ...
        <input
          ...
          accept={ACCEPT}
          className="sr-only"
          aria-describedby={validationError ? "inspect-file-error" : undefined}
          onChange={event => { onSelectFile(event.target.files?.[0] ?? null); event.target.value = ""; }}
        />
      </div>
      {validationError && <p className="error-message" role="alert" id="inspect-file-error">{validationError}</p>}
      <div className="example-row"><span>Try photographing</span><div>{exampleHints.map(hint => <span key={hint} className="inspect-example-chip">{hint}</span>)}</div></div>
    </> : <div className="inspect-preview">
      <div className="inspect-preview-image">{previewUrl && <Image src={previewUrl} alt="Uploaded item to inspect" width={520} height={340} unoptimized className="inspect-preview-img" />}</div>
      <div className="inspect-preview-actions">
        ...
        <Button type="button" className="inspect-analyze-button" disabled={isAnalyzing || disabled} onClick={onAnalyze}>{isAnalyzing ? <><LoaderCircle className="animate-spin motion-reduce:animate-none" />Analyzing image…</> : <>Analyze Image<ArrowRight /></>}</Button>
