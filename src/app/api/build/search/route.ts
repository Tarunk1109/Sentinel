...
export async function POST(request: Request): Promise<Response> {
  try {
    const parsed = buildSearchRequestSchema.safeParse(await readJson(request));
    ...
    if (!request.headers.get("accept")?.includes("application/x-ndjson")) {
      const built = await buildService.search(owner, parsed.data.planId, parsed.data.selectedIds, parsed.data.clarification, signal);
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        try { await buildService.search(owner, parsed.data.planId, parsed.data.selectedIds, parsed.data.clarification, signal, emit); }
