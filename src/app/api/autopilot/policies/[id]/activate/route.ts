...
/** DRAFT -> ACTIVE. Requires { "confirm": true }: a parsed voice draft can never activate itself. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  try {
    ...
    return Response.json({ autopilot: await autopilotService.activate(owner, id) }, { headers });
