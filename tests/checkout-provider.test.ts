...
describe('fulfillment and checkout pricing', () => {
  ...
  it('sends the authorized C$14.95 sandbox mandate constraint and blocks anything above it', async () => {
    const sandboxIntent = { ...intent, budget: { maxAmount: 14.95, currency: 'CAD' as const } };
    ...
    // Agnic authorizes this mandate at exactly 1495; a wider constraint is answered with a hosted-setup 202.
    expect(JSON.parse(String(allowed.network.mock.calls[0][1]?.body))).toMatchObject({ constraints: { max_total_minor: 1495 } });
    const blocked = setup({ ...quote, expected_amount_minor: 1496 });
    expect(await blocked.provider.previewOrder(product, sandboxIntent, context())).toMatchObject({ status: 'needs-setup', amount: null, budgetViolation: { limit: { amountMinor: 1495, currency: 'CAD' }, checkout: { amountMinor: 1496, currency: 'CAD' } } });
  });
